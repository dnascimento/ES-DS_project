...
public class PresentationServer {
	...
	private void doTestReplicationIfAbleToReceveComunication() {
		...
		try {
			...
			initCallCommand("910000000", "910000001");
		} catch (AnaComException e) {
			System.out.println(e.toString() + " " + e.getMessage());
		}

		try {
			System.out.println(" \n----------- 01 tenta terminar a chamada (devolve expção - receiver nao pode terminar a chamada)-----------------");
			endCallCommand("910000001", "910000000");
		} catch (AnaComException e) {
			System.out.println(e.toString() + " " + e.getMessage());
		}

		try {
			System.out.println(" \n-----------Fazer outra chamada de 00 para 01 (exepção pois já tem uma chamada em curso)-----------------");
			initCallCommand("910000000", "910000001");
		} catch (AnaComException e) {
			System.out.println(e.toString() + " " + e.getMessage());
		}
		...
		try {
			System.out.println(" \n----------- Tentar iniciar chamada apartir do receiver (exepção pois já tem uma chamada em curso)-----------------");
			initCallCommand("910000001", "910000000");
		} catch (AnaComException e) {
			System.out.println(e.toString() + " " + e.getMessage());
		}
